var Bijous, EventEmitter, Klect, async, exports, getModuleName, loadModule, path, setResult, util, _,
...
loadModule = function(def, results, done) {
  def.module.call(null, this, results, (function(_this) {
    return function(error, result) {
      if (result) {
        setResult.call(_this, def, results, result);
      }
      if (!error) {
        _this.emit('loaded', def.name, def.bundle, results);
      }
      return done(error);
    };
  })(this));
...
setResult = function(def, results, result) {
  var _name;
    ...
    fns = this.require(bundle).map((function(_this) {
      return function(def) {
        return function(done) {
          return loadModule.call(_this, def, results, done);
        };
      };
    })(this));
