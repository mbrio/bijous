var Bijous, EventEmitter, Klect, camelize, exports, getModuleName, loadModule, parameterize, path, series, setService, util, _,
...
getModuleName = function(file) {
  ...
  return camelize(parameterize(path.basename(file, extname)));
};

loadModule = function(def, services, done) {
  var isAsync, moduleDone;
  isAsync = def.module.length > 2;
  moduleDone = (function(_this) {
  ...
  if (isAsync) {
    def.module.call(null, this, services, moduleDone);
  } else {
    def.module.call(null, this, services);
    moduleDone();
  }
};
